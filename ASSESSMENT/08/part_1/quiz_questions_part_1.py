...
def create_group_questions(quiz_group):
    for i in range(200,301):
        ...
        question_text = f"""
            <p>You repeat your Zebrafish experiment and generate a new set of data.</p>
            <p>Create a new R project.</p>
            <p>Create a new R script file.</p>
            <p>Read the data directly to a variable (without downloading!) using the following url:</p>
            <p>https://raw.githubusercontent.com/rtreharne/qs/main/data/02/zebrafish_{i}.csv</p>
            <p>Perform a one-way ANOVA test, in R,  to determine if there is a significant difference in the length values between the ethanol concentration groups.</p>
