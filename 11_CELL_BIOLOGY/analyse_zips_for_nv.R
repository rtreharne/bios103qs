...
for (i in 1:length(zip_paths)) {
  ...
  for (j in 1:length(image_files)) {
    counts[j] <- proportion_nonviable(image_files[j])
  }
  
  positive[i] <- sum(counts > 20)
  
  print(paste("Positive results", positive[i]))

}

# Create a datatable with the zip filename and the mean number of cells
results <- data.frame(zip_paths, means)

# Save as a .csv file
write.csv(results, "nv_results.csv", row.names = FALSE)
